...
module.exports = async (client, message) => {
  try {
     ...
     if (cmd.length === 0){
      if(matchedPrefix.includes(client.user.id))
        return message.channel.send(new Discord.MessageEmbed()
          ...
          .setTitle(`DBC Ping recieve`)
          .setDescription(`Hello , I am DBC. In order to see all my commands , type \`${prefix}help\``)
        );
      return;
      }
    ...
    return message.channel.send(new Discord.MessageEmbed()
      ...
      .setDescription(`The command you tried to run is not a valid command. Please type \`${prefix}help\` for a full list of commands.`)
    ).then(msg=>msg.delete({timeout: 5000}).catch(e=>console.log("Couldn't Delete --> Ignore".gray)));
  }catch (e){
